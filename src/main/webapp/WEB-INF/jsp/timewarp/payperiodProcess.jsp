...
<s:if test="action != ''">
	...
	<s:if test="hasProcesses()">
			<table width="100%">
				...
				<s:iterator value="processes" var="process" >
					<tr>
						<td valign="top" class="td_text" id="<s:property value='employee.id' />"> <s:property value="employee" /> (<s:property value="employee.employee_number " />)</td>
						<td valign="top">
	...
	<s:if test="hasNoDataEmployees()" >
		<table width="50%">
			...
			<s:iterator value="noDataEmployees" status="row">
				<tr><td class="td_text" align="right"><s:property value="#row.index+1" /> - </td><td class="td_text">&nbsp;&nbsp;<s:property value="user.full_name" /></td></tr>
			</s:iterator>
		</table>
	</s:if>
</s:if>
