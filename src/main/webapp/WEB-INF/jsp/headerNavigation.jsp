        ...
        <nav class="dropdown" tabindex="0">
          ...
          <div class="links" aria-labeledby="genericMenuLauncher" hidden>
            <s:if test="!isUserCurrentEmployee()">
              ...
              <a href="<s:property value='#application.url' />timeDetails.action?employee_id=<s:property value='user.id' />&action=Change" tabindex="1">Stop User Proxy</a>
              <hr>
            </s:if>

            <s:if test="#session.user.isAdmin()">
              <a href="<s:property value='#application.url'/>settings.action" tabindex="1">Settings</a>
            </s:if>
            <a href="<s:property value='#application.url'/>Logout" tabindex="1">Logout</a>
          </div>
        </nav>
      </s:if>
    </div>
