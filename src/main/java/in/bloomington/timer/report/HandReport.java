...
/*
	...
	// HR OT report
	//
   select tt.empnum,tt.date,tt.code,tt.hours                                       from (select                                                                    concat_ws(' ',e.first_name,e.last_name) AS name,                                e.employee_number as empnum,                                                    t.date AS date,                                                                 c.name AS code, 							           t.hours AS hours                                                                from time_blocks t                                                              join hour_codes c on t.hour_code_id=c.id 				           join time_documents d on d.id=t.document_id                                     join pay_periods p on p.id=d.pay_period_id                                      join employees e on d.employee_id=e.id                                          where t.inactive is null                                                        and p.start_date >= str_to_date('12/18/2022','%m/%d/%Y')                        and p.end_date <= str_to_date('07/09/2023','%m/%d/%Y')                          and t.hour_code_id in (78,43,118,44,65,127,128,66,116,112,113,16,124,47,125,121)) tt                                                                            order by tt.empnum,tt.date                                                      INTO OUTFILE '/var/lib/mysql-files/hr_report3.csv'                              FIELDS TERMINATED BY ','                                                        ENCLOSED BY '"'                                                                 LINES TERMINATED BY '\n';
