...
public class GroupEmployee extends CommonInc implements Serializable{
    ...
    /**
     * 
     * employee group can NOT be delete if
     * 1-does not have id
     * 2-it is the only one for this employee
     * 3-it is only one with the same employee and group and has
     * has related job
     */
	...
	ResultSet rs = null;
	String qq = " select count(*) from group_employees where employee_id=? ";
	String qq2 = " select count(*) from group_employees where employee_id=? and group_id=? ";
	String qq3 = " select count(*) from jobs where employee_id=? and group_id=?";
	if(id.isEmpty() || employee_id.isEmpty() || group_id.isEmpty()){
	    return false;
	}
	...
	try{
		...
		pstmt2.setString(2, group_id);						
		rs = pstmt2.executeQuery();
		if(rs.next()){
		    cnt2 = rs.getInt(1);
		}
		if(cnt2 == 1){ // check if it has related job
		    qq=qq3;
		    pstmt3 = con.prepareStatement(qq3);
		    pstmt3.setString(1, employee_id);
		    pstmt3.setString(2, group_id);						
    ...
    //
    public String doExpireDateOnly(){
	String msg = "";
	Connection con = null;
