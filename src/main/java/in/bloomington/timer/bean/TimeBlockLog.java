...
public class TimeBlockLog extends Block{
	...
	super(val, val2, val3,
	      val4,
	      val5, val6, val7, val8, val9, val10, val11, val12, val13, val14, val16);
	setTime_block_id(val15);
	setAction_type(val17);
	setAction_by_id(val18);
	setAction_time(val19);
	...
	ResultSet rs = null;
	String msg="", str="";
	String qq = "select id,document_id,hour_code_id,earn_code_reason_id,date_format(date,'%m/%d/%Y'),begin_hour,begin_minute,end_hour,end_minute,hours,minutes,amount,clock_in,clock_out,notes,time_block_id,action_type,action_by_id,date_format(action_time,'%m/%d/%y %H:%i'),location_id from time_block_logs where id =? ";
	logger.debug(qq);
	...
	try{
		...
		setVals(
			...
			rs.getString(14),
			rs.getString(15));
		setTime_block_id(rs.getString(16));
		setAction_type(rs.getString(17));
		setAction_by_id(rs.getString(18));
		setAction_time(rs.getString(19));
		setLocation_id(rs.getString(20));
	    }
	}
	catch(Exception ex){
	    msg += " "+ex;
