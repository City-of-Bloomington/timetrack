...
// NOT used yet
// we try to optimize this code so that it can run faster
//
public class ProcessAction extends TopAction{

    static final long serialVersionUID = 2260L;	
    static Logger logger = LogManager.getLogger(ProcessAction.class);
    //
  ...
  public boolean hasNotApprovedMap(){
    ...
    private String findNotProcessed(){
	String back = "";
	if(hasGroups()){
	    if(pay_period_id.isEmpty()){
		getPay_period_id(); // current
